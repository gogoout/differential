...
Status: accepted, implemented (extends 0018; constrains 0002, 0011, 0012)

The migration landed over several changes and is complete: `NOT_YET_INVERTED` in
`crates/engine/tests/layering.rs` is empty, so that test now states the rule
unconditionally rather than tracking remaining debt.
