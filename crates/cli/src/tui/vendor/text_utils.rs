// Adapted from agavra/tuicr (0dacb6b), src/ui/text_utils.rs — near-verbatim
// (visibility widened to pub). MIT License — Copyright (c) 2025 tuicr
// contributors. See LICENSE-MIT.
#![allow(dead_code)] // vendored as a set; not every helper is used yet
...
pub fn truncate_or_pad_pairs_by_chars(
    pairs: &[(Style, String)],
    width: usize,
    base_style: Style,
) -> Vec<Span<'static>> {
...
pub fn contains_fold(text: &str, needle_folded: &str) -> bool {
...
pub fn apply_search_highlight_pairs(
    pairs: &[(Style, String)],
    needle_lower: &str,
    highlight: Style,
...
pub fn apply_search_highlight_text(
    text: &str,
    style: Style,
    needle_lower: &str,
    highlight: Style,
...
pub fn apply_search_highlight_spans(
...
pub fn wrap_spans<'a>(spans: &[Span<'a>], width: usize) -> Vec<Vec<Span<'a>>> {
    if width == 0 {
