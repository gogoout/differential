...
use differential_schema::{Effort, PlanDocument, ReadAction, SourceKind};
...
impl FakeBackend {
    fn new(name: &str, respond: impl Fn(&[String]) -> String + Send + Sync + 'static) -> Self {
        FakeBackend {
            name: name.to_string(),
...
fn json_group(label: &str, effort: &str, classes: &[&str]) -> String {
    format!(
...
fn grouped(r: &TestRepo, base: &str, head: &str, backend: &dyn LlmBackend) -> PlanDocument {
    grouped_with_cache(r, base, head, backend, None)
}

fn grouped_with_cache(
    r: &TestRepo,
    base: &str,
    head: &str,
